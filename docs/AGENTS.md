...
Build 7.3. This started as a plan for sixteen agents. Building them made the honest number clear:
**four have genuine agentic structure, and the other twelve are single model calls that
`verify.py`, `changes.py`, `coverage.py`, `lanes.py` and their neighbours already do well.**
Wrapping the twelve in `LlmAgent` for the count is exactly the padding `docs/DECISIONS.md` and the
people-finder-stays-not-an-agent call exist to refuse, so they are not wrapped. This document is
the design of record, and the section "Why not the other twelve" is part of it.

The lane check was the last to be demoted. Built as an agent, it cost a full ADK session per page
and would have made a watch round take hours. Deciding which route a page is about is one
judgment, not multi-step work, so it is now one `call_json` in `migragent/lanes.py` with the quote
check in code. Same judgment, no session.
...
| --- | --- | --- |
| **Lane check** | which route(s) a discovered page is about | one call, quote-checked in code (`migragent/lanes.py`). Built as an agent; a session per page made a watch round take hours, and it never navigated or revised meaningfully. Still closes D29 and D32. |
...
If a judge counts nodes, the honest count is four `LlmAgent`s, plus the orchestration below, plus
...
agent added nothing. This refusal sits next to the twelve above, as the same kind of call: a
deliberate no.

## The orchestration layer

**SequentialAgent, two, over the four real agents and the deterministic gates:**

- `IngestionPipeline`: Scout, then `RobotsGate`, then fetch and hash and snapshot, then Extractor,
  then the Verifier call, then the lane check, then `QuoteCheck`, then `CitationBinder`, then
  persist.
...
Four `LlmAgent`s, two `SequentialAgent`s, four `BaseAgent` gates. The number is small because the
...
- **Scout, Extractor, Coverage Matcher** as `LlmAgent`s. The Researcher was already there.
- **Lane check** as one `call_json` in `migragent/lanes.py`, wired into `round.py` at depth 1 and
  deeper. It was an agent for a day; a session per page made a watch round take hours.
- Extractor and Coverage Matcher swapped in by flag at their call sites.

Open:

1. `IngestionPipeline` and `IntakePipeline` as `SequentialAgent`s over the four agents, the lane
   check and the four gate nodes.
2. `docs/ARCHITECTURE.md` diagram: four agents, two sequences, four gates, and the twelve calls
   that are deliberately not agents.
3. Build 6: the video, the repo going public, the Devpost category.
